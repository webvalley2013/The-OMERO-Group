...
if __name__ == "__main__":
	...
	client = scripts.client('Upload Image', """Using this script, you can upload one or more images to a remote sever.""",
	    	scripts.String("Data_Type", optional=False, grouping="1", description="Choose source of images (only Image supported)", values=dataTypes, default="Image"),
			scripts.List("IDs", optional=False, grouping="2", description="List of Image IDs to change annotations for.").ofType(rlong(0)),
			scripts.String("Control_Tag", optional=False, grouping="3", description="Tag used for the control drug."),
			version = "0.1",
			...
			for annotation in image.listAnnotations():
				if isinstance(annotation, omero.gateway.TagAnnotationWrapper):
					if (annotation.getValue() == scriptParams["Control_Tag"]):
						...
						r = requests.put(url='http://192.168.205.10/owncloud/files/webdav.php/OmeroTest/ImageTest.tiff', data=omeTiffImage, auth=('webvalley','webvalley'))
						#omeTiffImage = image.exportOmeTiff()
						#payload = {'file': omeTiffImage}
